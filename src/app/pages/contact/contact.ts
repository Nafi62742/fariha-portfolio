...
export class ContactComponent {
  ...
  public readonly contactInfo: Array<ContactInfoItem> = [
    { icon: 'fa-envelope',     label: 'Email',    value: 'farihahassan.214@gmail.com', href: 'https://mail.google.com/mail/?view=cm&fs=1&to=farihahassan.214@gmail.com' },
    { icon: 'fa-phone',        label: 'Phone',    value: '+8801715891422',          href: 'tel:+8801715891422'            },
   ...
   * Loads the access key from the assets configuration file.
   */
  public onSubmit(): void {
    ...
    fetch('/assets/config/credentials.json')
      .then((res) => {
        if (!res.ok) {
          throw new Error('Failed to load credentials configuration.');
        }
        return res.json() as Promise<{ web3FormsAccessKey: string }>;
      })
      .then((config) => {
